...
/// A single-record SNI mutation: pure function from the raw TLS record
...
pub fn disguise_sni_record(
    record: &[u8],
    new_type: u16,
) -> Result<Vec<u8>, crate::error::DpiGuardError> {
...
mod tests {
    ...
    #[test]
    fn underscore_swap() {
        assert_eq!(inject_underscore(b"my-site.com"), b"my_site.com".to_vec());
    }
    ...
    fn profile_contents() {
        ...
        assert_eq!(
            get_mutation_profile(MutationProfile::ChinaRegional).len(),
            2
        );
    ...
    fn nested_cloak_profile_flags() {
        assert_eq!(MutationProfile::NestedCloak.recommended_fragment_size(), 32);
    ...
    fn new_profiles_have_small_chunks_and_disorder() {
        ...
        assert_eq!(
            MutationProfile::ChinaRegional.recommended_fragment_size(),
            32
        );
    ...
    fn disguise_type_is_grease_or_private() {
        for _ in 0..20 {
            ...
            let is_private =
                (SNI_DISGUISE_PRIVATE_RANGE.0..=SNI_DISGUISE_PRIVATE_RANGE.1).contains(&t);
