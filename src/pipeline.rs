...
mod tests {
    ...
    fn all_ports_custom_intercept_works() {
        ...
        let WireAction::Send(pkts) = action2 else {
            panic!()
        };
    ...
    fn relay_flow_injects_fake_hello_then_passes_through() {
        ...
        assert!(
            matches!(p.handle(&syn).unwrap(), WireAction::Send(v) if v.len() == 1 && v[0] == syn)
        );
        let synack = tcp_pkt_with_ack(
            [1, 1, 1, 1],
            [10, 0, 0, 1],
        ...
        assert!(
            matches!(p.handle(&dupack).unwrap(), WireAction::Send(v) if v.len() == 1 && v[0] == dupack)
        );
        ...
        assert!(
            matches!(p.handle(&real).unwrap(), WireAction::Send(v) if v.len() == 1 && v[0] == real)
        );
    }
